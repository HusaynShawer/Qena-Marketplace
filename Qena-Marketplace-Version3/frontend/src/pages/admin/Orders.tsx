    ...
    >
      <div className="bg-white rounded-2xl shadow-xl w-full max-w-md p-6 relative">
        ...
        <h2 className="text-lg font-bold text-gray-900 mb-1">
          Buyer Info — Order #{order.id}
        </h2>
        <p className="text-sm text-gray-400 mb-5">
          Seller: <span className="font-medium text-gray-600">{order.seller_name}</span>
        </p>
...
const AdminOrders: React.FC = () => {
  ...
  const handleStatusChange = async (orderId: string, newStatus: string) => {
    setUpdating(orderId);
    try {
      await api.patch(`/orders/seller/${orderId}/status`, { status: newStatus });
            ...
            {orders.map((order) => (
              <tr key={order.id} className="hover:bg-gray-50 transition">
                ...
                <td className="px-4 py-3 text-gray-700">
                  ...
                  <div className="text-xs text-gray-400">{order.seller_id?.slice(0, 8) || ""}</div>
                </td>
